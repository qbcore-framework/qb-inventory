    ...
    if dist<3 then
        if amount <= item.amount then
            ...
            if OtherPlayer.Functions.AddItem(item.name, amount, false, item.info) then
                ...
                TriggerClientEvent('QBCore:Notify', target, "You Recieved " .. amount .. ' ' .. item.label .. "!")
                Player.Functions.RemoveItem(item.name, amount, item.slot)
                TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove")
                TriggerClientEvent('QBCore:Notify', src, "You gave " .. OtherPlayer.PlayerData.name .. " " .. amount .. " " .. item.label .."!")
            else
